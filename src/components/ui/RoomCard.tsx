    ...
    return (
        <div className='relative w-full h-60'>
            <div className="absolute top-0 left-0 w-full">
                <Image className='max-h-60 rounded-md border border-neutral-700' src={room.category.imageUrl} height={1000} width={1000} alt={room.name} />
            </div>
            ...
            <div className="py-4 absolute w-full h-full top-0 left-0 z-50 flex flex-col justify-between">
                <div className="flex justify-between items-center">
                    <div className="bg-neutral-900 rounded-r-md p-4 pr-8 flex flex-col gap-1 max-w-fit">
                        ...
                        <p className="font-medium text-sm text-[#ffd12b]">{room.category.name}</p>
                    </div>
                ...
                {userInRoom ? (
                    <a className={cn(buttonVariants({ variant: "cta", size: "sm", className: "px-6 ml-auto mr-4", }))} href={`/rooms/${room.id}`}>Enter</a>
                ) : (
